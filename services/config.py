...
DEFAULT_REGISTER_POSTPROCESS = {
    "enabled": False,
    "export_local": False,           # 注册入库：导出到本地 data/account.txt（可与 push_sub2api 同时开）
    "push_sub2api": True,            # 注册入库：join 空间并推送 sub2api（原行为，可与 export_local 同时开）
    "local_membership_type": "K12",  # 本地导出写入 account.txt 的会员类型字段（email----password----会员类型----接码URL）
...
def _normalize_register_postprocess_settings(value: object) -> dict[str, object]:
    ...
    # 注册入库目标：export_local(本地 account.txt)/push_sub2api(推送 sub2api)，两者可同时开。
    # 兼容旧单选 storage_mode：新字段都缺省时按旧值迁移（local→仅本地导出；其余非空→仅推 sub2api）。
    if "export_local" in source or "push_sub2api" in source:
        export_local = _normalize_bool(source.get("export_local"), False)
        push_sub2api = _normalize_bool(source.get("push_sub2api"), True)
    elif str(source.get("storage_mode") or "").strip().lower() == "local":
        export_local, push_sub2api = True, False
    elif str(source.get("storage_mode") or "").strip():
        export_local, push_sub2api = False, True
    else:
        export_local = bool(DEFAULT_REGISTER_POSTPROCESS["export_local"])
        push_sub2api = bool(DEFAULT_REGISTER_POSTPROCESS["push_sub2api"])
    return {
        "enabled": _normalize_bool(source.get("enabled"), False),
