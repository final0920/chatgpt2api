...
def _parse_outlook007_pool_with_report(text: str, allow_schemeless: bool = False, default_scheme: str = "https") -> tuple[list[dict[str, str]], dict[str, Any]]:
    ...
    for line_no, raw_line in enumerate(str(text or "").splitlines(), start=1):
        ...
        if not (code_api_url.startswith("http://") or code_api_url.startswith("https://")):
            if allow_schemeless and code_api_url and "." in code_api_url:
                code_api_url = f"{default_scheme}://" + code_api_url  # 省略 scheme 时按渠道默认补全（smsbower→https，gmcode→http）
            else:
                report["invalid"] += 1
...
# ---- 母箱轮询 + 失败冷却调度（smsbower / gmcode 共用）----
# 问题：create_mailbox 原按 pool 顺序选第一个未满母箱，且失败别名不落库（usage 只算注册成功数），
# 某母箱注册连续失败时 usage 恒为 0、永远排第一被连续选中，死磕坏母箱 → 429 / 接码错乱。
# 解法：独立于 usage 的内存态领用计数——
#   轮询：每次选"领用最少"的母箱，即每领用一个别名就轮换到下一个母箱（母箱间雨露均沾）；
#   冷却：母箱注册失败（超时/坏码/接码错乱等）后进冷却期，选箱时排最后 → 立刻改用别的母箱。
# 内存态即可：usage>=per 仍是硬上限（不会超发），重启后从头轮询也正确。
SMSBOWER_MOTHER_COOLDOWN = 180.0  # 母箱注册失败后的冷却秒数（期内优先跳过，改用别的母箱）
_smsbower_dispatch_guard = Lock()
_smsbower_dispatch: dict[str, dict[str, float]] = {}  # mother_key -> {"count": 领用累计, "cooldown_until": monotonic}


def _smsbower_dispatch_sortkey(mother_key: str, now: float) -> tuple[int, float]:
    """母箱选取排序键（越小越优先），调用方须持 _smsbower_dispatch_guard：
    (是否冷却中 0/1, 领用累计)。冷却中排最后；否则领用最少的母箱优先——
    每领用一个别名就轮换到下一个母箱，实现母箱间雨露均沾。"""
    st = _smsbower_dispatch.get(mother_key)
    if not st:
        return (0, 0.0)
    cooling = 1 if now < st.get("cooldown_until", 0.0) else 0
    return (cooling, st.get("count", 0.0))


def _smsbower_note_dispatch(mother_key: str) -> None:
    """领用一次母箱：累计计数 +1（成功失败都算，仅用于轮询排序）。"""
    with _smsbower_dispatch_guard:
        st = _smsbower_dispatch.setdefault(mother_key, {"count": 0.0, "cooldown_until": 0.0})
        st["count"] = st.get("count", 0.0) + 1


def _smsbower_note_cooldown(mother_key: str, seconds: float = SMSBOWER_MOTHER_COOLDOWN) -> None:
    """母箱注册失败：进入冷却期，此后 seconds 秒内选箱时排最后（优先改用别的母箱）。"""
    with _smsbower_dispatch_guard:
        st = _smsbower_dispatch.setdefault(mother_key, {"count": 0.0, "cooldown_until": 0.0})
        st["cooldown_until"] = time.monotonic() + max(0.0, seconds)


def _is_valid_otp(code: str | None) -> bool:
    """合法 OpenAI 邮箱注册 OTP：6 位纯数字。
    接码源偶发返回非 6 位（主人实测 gmcode 有时返回 5 位）即坏码，不能用于提交——
    尤其不能补前导 0 硬凑：API 返回的 5 位本身就是错的，补 0 提交照样 otp 错误，
    正确做法是判坏码后放弃该母箱、轮询到下个母箱（见 wait_for_code）。"""
...
def reset_smsbower_pool_state() -> int:
    ...
    with _smsbower_dispatch_guard:
        _smsbower_dispatch.clear()  # 清空轮询/冷却调度，重置雨露均沾
    with _smsbower_locks_guard:
...
class SmsbowerGmailProvider(BaseMailProvider):
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        # 轮询 + 冷却：选"领用最少"的母箱，每领用一个别名就轮换到下个母箱（雨露均沾）；
        # 失败母箱进冷却期排到最后（改用别的母箱）。领用计数独立于 usage（失败别名不落库，usage 不增）。
        now = time.monotonic()
        with _smsbower_dispatch_guard:
            ordered = sorted(
                self.pool,
                key=lambda c: _smsbower_dispatch_sortkey(_smsbower_mother_key(str(c.get("email") or "")), now),
            )
        for cred in ordered:
            ...
            try:
                ...
                _smsbower_note_dispatch(mother_key)  # 领用成功：轮询计数 +1（下次该母箱轮次靠后）
                return {
                    "provider": self.name,
                    "provider_ref": self.provider_ref,
                    "address": alias,
    ...
    def wait_for_code(self, mailbox: dict[str, Any]) -> str | None:
        ...
        while time.monotonic() < deadline:
            ...
            if ok:
                ...
                if new_codes:
                    candidate = new_codes[-1]  # all_codes 时间升序，取最新的新增码
                    # 接码源偶发返回非 6 位数字（主人实测 gmcode 有时返回 5 位）= 坏码：
                    # 立即放弃当前母箱（返回 None → 上层失败 → 母箱进冷却 → 轮询到下个母箱）
                    if not _is_valid_otp(candidate):
                        return None
                    # 复查确认码稳定，避免接码错乱（拿到上一别名残留的旧码）导致 otp 错误：
                    # 隔 2s 再查，最多 3 次，须持续读到同一新码才采用
                    if self._confirm_code_stable(code_api_url, baseline, candidate):
                        return candidate
            time.sleep(max(0.2, self.conf["wait_interval"]))
        return None

    def _confirm_code_stable(self, code_api_url: str, baseline: set[str], candidate: str,
                             rounds: int = 3, interval: float = 2.0) -> bool:
        """复查确认候选新码稳定：隔 interval 秒重读，最多 rounds 次，
        每次都须仍读到同一个 candidate（作为最新新码）才算稳定。
        避免接码源瞬时错乱（返回上一别名残留的旧码）导致用错码提交。"""
        for _ in range(rounds):
            time.sleep(interval)
            ok, codes = self._read_all_codes(code_api_url)
            if not ok:
                return False
            fresh = [c for c in codes if c and c not in baseline]
            if not fresh or fresh[-1] != candidate:
                return False
        return True
...
def smsbower_reauth_prepare(mailbox: dict[str, Any], mail_config: dict) -> None:
    ...
    prov = None
    try:
        # 按 mailbox 实际渠道创建解析器（smsbower_gmail / gmcode_gmail 取码 JSON 结构不同），
...
# ==================== gmcode-gmail 接码渠道（api.gmcode.vip）====================
# 与 smsbower-gmail 同构（母邮箱----取件URL，gmail 加号别名，母箱串行 + baseline 差异取码，
# 别名不落库用已注册账号数控上限），两处差异：
...
def parse_gmcode_gmail_pool(text: str) -> list[dict[str, str]]:
    """母箱池格式：每行 母邮箱----取件URL。gmcode 只支持 http，故省略 scheme 时默认补 http。"""
    return _parse_outlook007_pool_with_report(text, allow_schemeless=True, default_scheme="http")[0]


def inspect_gmcode_gmail_pool(text: str) -> dict[str, Any]:
    return _parse_outlook007_pool_with_report(text, allow_schemeless=True, default_scheme="http")[1]


def gmcode_gmail_pool_stats(pool: list[dict[str, str]] | None = None,
                            per_mother: int = GMCODE_ALIAS_PER_MOTHER) -> dict[str, int]:
    """统计 gmcode-gmail 母箱池额度。语义与 smsbower 完全一致（母箱数×额度−已注册数），
    锁/账号扫描共用同一套（gmail 母箱按 email 归一化，天然隔离），故直接委托。"""
    return smsbower_gmail_pool_stats(pool, per_mother)


class GmcodeGmailProvider(SmsbowerGmailProvider):
    """gmcode-gmail 接码渠道（api.gmcode.vip）：与 smsbower-gmail 同构，仅取件 URL 返回结构不同。

    取件 URL 返回 {"time":"..","code":"succeed","data":"305536"}：顶层 code 是状态串（succeed），
    验证码直接在 data 字符串里。父类 smsbower 的 _read_all_codes 会把顶层 code 误当验证码，
    故这里重写只取 data。其余 create_mailbox / wait_for_code / baseline / 母箱锁全部复用父类。
    默认每母箱分裂 5 个加号别名（GMCODE_ALIAS_PER_MOTHER，父类 smsbower 默认 7）。
    """
    ...
    def __init__(self, entry: dict, conf: dict):
        ...
        # entry 未显式指定 alias_per_email 时用 gmcode 默认 5（父类已按 smsbower 默认 7 赋过，这里覆盖）
        self.per_mother = _normalize_int(entry.get("alias_per_email"), GMCODE_ALIAS_PER_MOTHER, 1, 200)

    def _read_all_codes(self, code_api_url: str) -> tuple[bool, list[str]]:
        """GET 取件 URL，从 data 取验证码，返回 (ok, [code])。ok=False 表示请求/解析失败（可重试）。

        gmcode 返回 {"time":"..","code":"succeed","data":"305536"}，验证码在 data（字符串），
        顶层 code 是状态串不能当码。每次只返回最新单码，包成单元素列表复用父类 baseline 差异取码：
        ...
        if isinstance(inner, bool):
            code = ""  # bool 是 int 子类，先排除避免误当数字
        elif isinstance(inner, str):
            code = inner.strip()  # gmcode：验证码直接在 data 字符串
        elif isinstance(inner, int):
            code = str(inner)  # 兼容 data 为数字：可能已丢前导 0，出口 normalize_otp_code 会补回
        elif isinstance(inner, dict):
            code = str(inner.get("code") or "").strip()  # 兼容 data 为对象的变体
...
def _create_provider(mail_config: dict, provider: str = "", provider_ref: str = "") -> BaseMailProvider:
    ...
    if entry["type"] == "gmcode_gmail":
        return GmcodeGmailProvider(entry, conf)
...
def mark_mailbox_result(mailbox: dict, *, success: bool, error: Exception | str | None = None) -> None:
    ...
    if provider_name in {SmsbowerGmailProvider.name, GmcodeGmailProvider.name}:
        ...
        mother_key = str(mailbox.get("_smsbower_mother_key") or "")
        if not success and mother_key:
            # 注册失败且 OpenAI 报"该邮箱已存在"→ 整个 gmail 母箱在 OpenAI 侧已满，标记耗尽：无论分裂数设多少都不再分裂
            if _is_user_already_exists_error(error):
                mark_gmail_mother_exhausted(mother_key, str(error or ""))
            else:
                # 其他失败（等待验证码超时/接码错乱等）→ 母箱进冷却期，选箱时排最后，立刻改用别的母箱
                _smsbower_note_cooldown(mother_key)
        if mother_key:
            lock = _smsbower_mother_locks.get(mother_key)
            if lock is not None:
                try:
